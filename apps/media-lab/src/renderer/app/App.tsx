...
function ReviewExport(): JSX.Element {
  ...
  const [proposal, setProposal] = useState<EditProposalV1 | null>(null);
  ...
  const propose = async () => {
    ...
    setProposing(true);
    setChatError(null);
    try { setProposal(await window.vpMedia.chat.propose({ projectId: active.id, message })); setMessage(''); }
    catch (error) { setChatError(error instanceof Error ? error.message : 'Unable to request an editing proposal.'); }
    finally { setProposing(false); }
  };
  return <section className="review-layout"><div className="video-placeholder tall">Vertical preview | 9:16</div><div className="card"><span className="eyebrow">EDITING DRAFT</span><h2>{active?.title ?? 'Choose a local draft'}</h2>{projects.length === 0 ? <p>Create a local draft in Create before reviewing or asking the Copilot to propose edits.</p> : <select value={active?.id ?? ''} onChange={(event) => void selectProject(event.target.value)}><option value="">Select draft</option>{projects.map((project) => <option key={project.id} value={project.id}>{project.title}</option>)}</select>}{active && <p>{active.storyboard.beats.length} beat(s) | {active.storyboard.language.toUpperCase()} | edits remain drafts until confirmed.</p>}<span className="eyebrow">EDITING COPILOT</span>{active && <><textarea value={message} onChange={(event) => setMessage(event.target.value)} placeholder="Describe one change, e.g. “make the first caption shorter”" rows={3} /><button className="secondary" disabled={proposing || message.trim().length === 0} onClick={() => void propose()}>{proposing ? 'Preparing proposal...' : 'Propose edit'}</button></>}{proposal && <div className="notice"><strong>Proposal only:</strong> {proposal.summary}<br />{proposal.operations.map((operation) => operation.type).join(', ')}. Review and confirmation are required before any edit is applied.</div>}{chatError && <div className="notice">Copilot unavailable: {chatError === 'AI_NOT_CONFIGURED' ? 'configure an eligible Model Studio service key in Settings.' : chatError}</div>}<span className="eyebrow">REVIEW GATE</span><div className="check">Every selected asset has current rights.</div><div className="check">Travel facts are supported by approved evidence.</div><div className="check">Reference content is not used in the render.</div><button className="primary" disabled>Export bundle</button></div></section>;
}
